...
func NewResponse(r *Request) *Response {
	return &Response{
		Message: 38 + int32(r.Body.TopicsArray.Topics[0].NameLength) - 1,
		Header: &ResponseHeader{
				...
				Topics: ResponseBodyTopics{
					&ResponseBodyTopic{
						...
						TopicName: &ResponseBodyTopicName{
							Length:   r.Body.TopicsArray.Topics[0].NameLength,
							Contents: r.Body.TopicsArray.Topics[0].Name,
						},
						TopicID:                   [16]byte{},
						IsInternal:                0,
						PartitionsArray:           1,
